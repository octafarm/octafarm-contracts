...
contract StrategyCakeLP is StratManager, FeeManager, GasThrottler {
    ...
    ) StratManager(_keeper, _strategist, _unirouter, _vault, _octaFeeRecipient) public {
        want = _want;
    ...
    function chargeFees() internal {
        uint256 harvested = IERC20(cake).balanceOf(address(this));
        console.log('Harvested', harvested);
        uint256 toWbnb = IERC20(cake).balanceOf(address(this)).mul(50).div(1000);
        ...
        uint256 octaFeeAmount = wbnbBal.mul(octaFee).div(MAX_FEE);
        IERC20(wbnb).safeTransfer(octaFeeRecipient, octaFeeAmount);
