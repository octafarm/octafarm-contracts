...
contract StrategyMdexBscLP is Ownable, Pausable, GasThrottler {
     ...
     * {octf} - OctaFarm.fi token, used to send funds to the treasury.
    ...
    address constant public output = address(0x9C65AB58d8d978DB963e63f2bfB7121627e3a739); // MDEX
    address constant public octf = address(0xCa3F508B8e4Dd382eE878A314789373D80A5190A);
    ...
    /**
     * @dev OctaFarm.fi Contracts:
     * {rewards} - Reward pool where the strategy fee earnings will go.
     * {treasury} - Address of the OctaFarm.fi treasury
     ...
     * {rewardsFee} - 3% that goes to OctaFarm holders. Can be increased by decreasing {callFee}.
     */
    ...
    constructor(
        ...
        address _vault,
        address _strategist,
        address[] memory _pathToLp0,
        address[] memory _pathToLp1
    ) public {
        lpPair = _lpPair;
        want = _lpPair;
        ...
        if (lpToken0 == wbnb || lpToken0 == busd || lpToken0 == usdt) {
            ...
            outputToLp0Route = _pathToLp0;
        }

        if (lpToken1 == wbnb || lpToken1 == busd || lpToken1 == usdt) {
            outputToLp1Route = [output, lpToken1];
        } else if (lpToken1 != output) {
            outputToLp1Route = _pathToLp1;
        }

        IERC20(lpPair).safeApprove(masterchef, type(uint128).max);
        IERC20(output).safeApprove(mdxrouter, type(uint128).max);
        IERC20(wbnb).safeApprove(unirouter, type(uint128).max);

        IERC20(lpToken0).safeApprove(mdxrouter, 0);
        IERC20(lpToken0).safeApprove(mdxrouter, type(uint128).max);

        IERC20(lpToken1).safeApprove(mdxrouter, 0);
        IERC20(lpToken1).safeApprove(mdxrouter, type(uint128).max);
    }
    ...
    function chargeFees() internal {
        ...
        uint256 treasuryFull = wbnbBal.mul(TREASURY_FEE).div(MAX_FEE);
        IERC20(wbnb).safeTransfer(treasury, treasuryFull);
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != output) {
            // console.log('path to 0', outputToLp0Route[0], outputToLp0Route[1], outputToLp0Route[2]);
